  ...
  describe "create role" do
    test "redirects to show when data is valid", %{conn: conn} do
      conn =
        conn
        |> FakeSession.sign_in()
        |> post(role_path(conn, :create), role: @create_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn =
        conn
        |> FakeSession.sign_in()
        |> post(role_path(conn, :create), role: @invalid_attrs)
  ...
  describe "update role" do
    ...
    test "redirects when data is valid", %{conn: conn, role: role} do
      conn =
        conn
        |> FakeSession.sign_in()
        |> put(role_path(conn, :update, role), role: @update_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn, role: role} do
      conn =
        conn
        |> FakeSession.sign_in()
        |> put(role_path(conn, :update, role), role: @invalid_attrs)
  ...
  describe "delete role" do
    ...
    test "deletes chosen role", %{conn: conn, role: role} do
      conn =
        conn
        |> FakeSession.sign_in()
        |> delete(role_path(conn, :delete, role))
